...
const generate = () => {
  ...
  if (passwordLength > 8 && passwordLength < 128) {
    ...
    alert(
      "Your password does not meet the requirements. Please refresh and try again."
    );
  }
  ...
  if (randomCharString !== "") {
    for (i = 1; i <= passwordLength; i++) {
      passwordString =
        passwordString +
        randomCharString.charAt(
          Math.floor(Math.random() * Math.floor(randomCharString.length - 1))
        );
    }
    ...
    alert(
      "You must use at least one kind of character. Please refresh and try again."
    );
  }
};
